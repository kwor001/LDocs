...
If your system blocks browser-to-IDE handoff, manually provide your export token to the extension. Open the Command Palette and run **LaunchDocs: Fetch docs with token…**. Paste the token obtained from the export URI and press Enter. The extension fetches and writes files as it would during a browser handoff. This is useful in environments where URI handlers are restricted.
